...
### Checks runs on the documents the suite reads (2026-09-25)

Audit finding G3. The `ci.yml` push filter named code paths only, while the suite reads
`IDEAS.md` (`test_ideas_index.py`'s caps), `README.md` and `docs/counts.md` (the generated
counts, the contact address), the archive files, and `tietosuoja/index.html`
(`check_inline_js.py`'s default list and the privacy tests). A push touching only one of
them ran no Checks, and the next code push went red for it: 78 of 223 maintainer commits
from 09-15 touched those files and no code path. The filter now adds `IDEAS.md`,
`README.md`, `docs/**` and `tietosuoja/**`. `data/**` stays out: the local half's data
commits land several times a day. `CLAUDE.md` is read by `test_ideas_index.py` and
`test_design_contract.py` as well and was not in the requested list, so it is left out and
named here. Verified by reading the filter; no push was made to test it.

Later the same day `CLAUDE.md` was added to the filter, closing that remainder: an edit to
the rules those two tests quote now runs Checks like any other document the suite reads.

### The unit suite runs one process per test file, in parallel (2026-09-25)

Audit, test time. The suite ran as one `unittest discover` process: 86 s locally on 3,346
tests, 82 s median on CI, 78 s of it inside tests and most of that waiting rather than CPU.
The audit ran it per file on 4 and 14 workers with 0 failing files, the tree clean after
each, under two unusual time zones and a dead proxy as well. `scripts/run_tests.py` runs
each `tests/test_*.py` as `python -m unittest discover -s tests -p <file>`, largest files
first, one per CPU by default, prints each file's output under a header and then the totals
in unittest's words (`Ran N tests`, `OK`/`FAILED (failures=.., skipped=..)`), so the Checks
step's greps for `FAIL:`/`ERROR:`, `skipped=` and `ResourceWarning` mean what they meant; a
file that exits non-zero or runs nothing fails the suite, and `--fail-on-skip` makes a skip
a failure. Measured here: 25.7 s over 4 workers on 3,438 tests, 0 skipped, with the worktree
venv. `ci.yml` uses it with `--fail-on-skip`, keeping its own skip grep; CLAUDE.md's Testing
section names the command. Stdlib only. Tests: `test_run_tests.py`, 4 tests over a temporary
directory; 4 mutations, all red (the checker scored one VOID because the nested run's output
carries `_FailedTest`; run by hand, it turned two tests red).

### A main push's Checks run is never cancelled (2026-09-25)

Audit finding G8. `ci.yml` grouped runs by ref with `cancel-in-progress`, so a second push
to main inside a minute cancelled the first run, and with it the design and CACHE gates,
which check only that push's `before..after` range. The newer run's range starts after the
cancelled commits, so an `index.html` change with no CACHE bump could reach main with the
newest Checks run green. `cancel-in-progress: false` would not close it: GitHub keeps one
pending run per group and cancels the one it replaces. A main push is now grouped by its
SHA, so each runs to the end; a branch keeps one group and cancels as before, and its
cancelled ranges are checked by the main push that fast-forwards over them. Verified by
reading the expression; a workflow's concurrency has no local test.

### IndexNow runs are not cancelled by a newer one (2026-09-25)

Audit finding G7, prior review #23. `indexnow.yml` had one concurrency group with
`cancel-in-progress`, on the stated premise that an older run's URL list is a subset of the
newer one's. It is not: a push run announces its own `before..after`, and a fetch-triggered
run announces the one data commit that run published (`bot_commit_for_run`). A page push
and a cloud run finishing inside the same ~12 s job lost the first range. The group is
removed rather than set to queue, because GitHub cancels a pending run a newer one
replaces. Triggered runs never cover the same commit, so nothing is submitted twice; a
hand dispatch announces `HEAD^..HEAD` and can repeat the last. `test_indexnow.py` passes
unchanged; the workflow change was verified by reading it.

### The fetch workflow's write token reaches the push alone (2026-09-25)

Audit finding G10, first half. `actions/checkout` stores the job's `contents: write` token
in `.git/config` by default, so every later step in `biorex.yml` could read it, including
the fetch, enrich and mirror steps that parse third-party pages. A parser bug, or a
compromised dependency in one of those steps, could have pushed to `main`. The checkout
now sets `persist-credentials: false`. "Commit data and logs" builds the same
`AUTHORIZATION: basic` header checkout would have stored, masks it, and passes it with
`git -c` to its own pull and push only, so it is never written to disk. The pull needs no
credential on a public repository. Checked with a local HTTP listener: the header arrives
as `basic` plus the base64 of `x-access-token:<token>`. The first cloud run after the push
is what proves the workflow.

### Pillow is pinned by hash in the fetch workflow (2026-09-25)

Audit finding G10, second half. `biorex.yml` installed `pillow==12.3.0` by version alone,
in the job that pushes to `main`, so an index or mirror serving a different file under that
name would run in the mirror step. The step now writes a requirements file with PyPI's
sha256 for the eight CPython x86_64 manylinux wheels of 12.3.0 (3.10 to 3.15, free-threaded
builds included, so a runner Python upgrade still matches) and installs it with
`--require-hashes --only-binary :all:`. Pillow has no runtime dependencies to hash. Checked
with `pip download --platform manylinux_2_28_x86_64 --python-version 3.12`: the wheel's
own sha256 matches, and the file with that hash removed is refused. `ci.yml` keeps its
version pin: its token is read-only. `test_mirror_posters.py` now requires the version,
a hash and `--require-hashes`; 3 mutations, all red.

### Access and ethics: a cinema-provided ticket link is published, a booking endpoint is not visited (2026-09-26)

On the maintainer's instruction, CLAUDE.md now separates publishing a ticket link the
cinema's own listing carries from calling a booking endpoint: the href is copied and
resolved, never constructed and never requested, not even once in a browser; a link that
could only come from calling such an endpoint or with an unissued credential stays out. The
link-check bullet under "Adding a provider" names the exception. What prompted it, Myyri
and Sheryl against Kilta, Laika and Orion, is recorded in `2026-09-providers.md`.
