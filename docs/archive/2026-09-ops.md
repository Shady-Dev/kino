...
Chromium and WebKit, `fail-fast: false`, both required. `KINO_BROWSER_ENGINE` carries the
engine to the test, the Playwright cache is keyed by engine as well as version, and the
failure artefact's name carries it too, because `upload-artifact` refuses two uploads under
one name.
