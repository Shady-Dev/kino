...
empty. The markup is unchanged. Inside the existing 560 px breakpoint `.film` becomes a
two-column grid and `.info` becomes `display:contents`, so the header parts are the grid's
own items in column two and the list spans both columns. The poster takes `grid-row:1/-2`,
every row but the list's, so it stays beside the title, the metadata and the synopsis
whatever of those a film has. Measured on the Helsinki city page: at 393 px the list went
