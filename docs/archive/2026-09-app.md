...
**Read with the entry of 2026-09-17 below**: the freeze this paragraph describes was
lifted case by case five times over the following day, and `IDEAS.md` under "The client is
edited when the maintainer asks" says what it means now. The account below is unchanged.
...
### Four faults in the sheet, and the freeze around it (2026-09-17, sw.js v171-v174)

A review of the whole project reported fourteen findings. Four were client faults and the
maintainer asked for all of them; each landed on its own with its own `sw.js` bump. They
are recorded together because they were one pass and because three of the four are the
same shape: something that was true when the sheet opened was not re-checked when the page
moved under it.
...
**What the freeze meant, since it is now five exceptions old.** The instruction of
2026-09-14 said `index.html` is frozen; the entry above this one describes it as standing
until lifted. In practice the maintainer has lifted it per change, in their own words, five
times. The rule that survives is that this repo does not decide on its own to edit the
client: a real bug is not an authorisation, and an audit finding is not one either. The
separate instruction that did not move, restated on 2026-09-17, is that no design changes
go with any of this -- `DESIGN.md` and `tests/test_design_contract.py` were untouched by
all five.
