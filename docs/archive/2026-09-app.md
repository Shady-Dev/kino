...
A review of the whole project reported fourteen findings. Four were client faults; each
landed on its own with its own `sw.js` bump. They are recorded together because they were
one pass and because three of the four are the same shape: something that was true when the
sheet opened was not re-checked when the page moved under it.

**Corrected 2026-09-17.** This entry was first written dating all four to 2026-09-17 and
saying the maintainer asked for all of them. The dates were wrong and are now in the
heading: by the committer clock in Helsinki, v171 is 2026-09-16 23:53 and v172 23:57, while
v173 is 2026-09-17 00:00 and v174 00:36. The second half was not the repo's to state --
see the closing section.

**A shared screening link marked its ticket and left it off screen (v171).** `#m=&d=`
scrolls the named day to the top of the sheet body, and the walk to that day's `<h3>`
started at the stub: `el.parentElement`. A stub's siblings are the other tickets of the
same day, so the walk ran out of siblings inside its own `.stubs` box and reached no
heading, leaving `h` null and the scroll unapplied. It starts at `el.closest('.stubs')`
now, whose previous siblings are the past-times box, the reveal button and then the
heading. Measured at 375x320: the marked ticket sat 87 px below the body.

**A background refresh took the reader's focus (v172).** `refreshOpenSheet` redraws an
open sheet when `films-extra.json` changes, and `showSheet` ends by focusing the close
button, which is right when a reader opens a sheet and wrong when one is redrawn under
them. `keepFocus` is a parameter rather than a test of whether the sheet is already open,
because a reader following a link to another film from an open sheet is opening a sheet
and must get the keyboard. The redraw replaces the markup, so the focused control is noted
before it and found again after by what named it -- beside the scroll position that path
already restored.

**A selection claimed the previous cinema's freshness (v173).** `state.generated` was
written only when the payload carried one, so a selection with none kept the last one's,
and the paint returned before hiding either the stale banner or the credit line. A
combined city whose every part failed is exactly that selection. Both are now written on
every load and hidden when there is nothing to say.

**A history step left a sheet over another venue (v174).** Its own entry is below.

**What `index.html` has done since the freeze, and what that does not establish.** The
instruction of 2026-09-14 said the file is frozen, and the entry above describes it as
standing until lifted. What this repo can show is the file's own history since, by the
committer clock in Helsinki:
...
**That list is not evidence that any of them was authorised, and this file does not treat
it as any.** An earlier draft said each was asked for in the maintainer's own words; the
authorisations, if they were given, were given in conversation, which this repo does not
hold and cannot check. CLAUDE.md is explicit about both halves of that -- verify a claim
before documenting it, and say plainly when a fact cannot be checked -- so the claim is
withdrawn rather than softened, and nothing here declares the freeze lifted, narrowed or
still in force. A commit having landed says only that it landed.

What is on record and did not move is separate: no design change went with any of these.
`DESIGN.md` and `tests/test_design_contract.py` are untouched by all six, which is checked
rather than asserted -- `git diff` over that range names neither file.
...
reach, each saying which and why. Sixteen mutations across the four, all red. One test that
pinned `sw.js` at exactly `v170` became a floor: it asserted the date rather than the rule
and failed the next client change for a reason that had nothing to do with synopses.
...
`onPopState` now says what happens to the sheet in the branch that needs it. Leaving a
venue closes it **before** the new schedule is asked for, so the wrong film is never on
screen beside the right page, and reopens it from the entry's own fragment once that
venue's shows are in, where the film resolves against that venue's data and the sheet says
so when it has none. The step to the chooser closes it. A hash-only step is left alone:
`hashchange` fires for it and owns the fragment, and doing it here as well redraws the
sheet twice for one step, which a test had already pinned as changing nothing.

**Three branches were written and then removed for being unobservable**, rather than left
untested: closing the sheet in the branch where nothing is selected at all, making
`syncSheet` close it when there is no area, and acting only on a disagreement between the
fragment and the open sheet. Only venue picks push history entries -- every other URL
change is a `replaceState` -- so a same-area traversal always carries a fragment change,
and the chooser branch above already closes the sheet.

The fixture gained a second venue for this, Promenadi Pori, showing a film no other venue
in it does: a sheet left over from one cinema is then visible in the other's page by its
title. The mid-load window needed more than that. A traversal refetches nothing, because
the venue returned to is already in `jsonCache`; a reload empties the cache and keeps the
history, and the file still came from the browser's own HTTP cache, so the fixture answers
a delayed file `no-store` as well. Only then is there a second of flight to observe, and
the ordering mutation goes red.
...
reopen. It is what makes the claim above true -- that the sheet comes back only once *that*
venue's shows are in -- and dropping it left every test green. It is exercised now by a
venue file released after the reader has gone back to the chooser: the entry's own `#m=`
would otherwise open that venue's film, modal, over a chooser it has nothing to do with.
The counterweight scenario, the same load arriving while the reader is still on the venue
that named it, is asserted beside it so the guard cannot be satisfied by never reopening.
Three mutations, all red: the guard dropped, the reopen removed, and the comparison
inverted.
...
The correction is the guard the focus restore already had: `if(wasOpen)`. A modal clears
only what it set, which is what makes three of them safe to share one flag. Gating the two
new calls in `onPopState` instead would have left the next unconditional caller to
rediscover this.

Found by review of `25a7cc38` rather than in use, and measured in a real browser before it
was believed: at that commit the picker stays open with nothing inert behind it, and at its
parent all three stay inert.

Tests: three in `tests/browser/test_client_browser.py` -- Back with the picker open, the
ordinary close still handing the page back afterwards, and a traversal with no modal open
leaving the page alone. Two mutations: the unconditional clear restored, which reds the
first; and the clear removed altogether, which reds five, so the guard cannot be satisfied
by never clearing at all.
