...
### The search finds a genre by its Swedish name (2026-09-25, v250)

Audit finding K8. `haystack()` added the TMDB genre names in Finnish and English only,
while a Swedish card shows the Swedish one: "Äventyr" found nothing and "adventure" found
the film. 14 of the 19 Swedish names in `data/tmdb-genres.json` are neither the Finnish
nor the English name. The haystack now takes all three languages, as the venue picker's
haystack already does for names. No visual change. `GenreSearchInSwedish` (1200 and 375 px)
serves the committed genre file and finds Orion's "Oasis: Don" by "äventyr" in Chromium
and WebKit; 1 mutation, red in both.

### The app sets its theme in <head> (2026-09-25, v251)

Audit finding K9. `index.html` applied the stored theme from the script at the end of the
body, which runs only once all of the document has parsed; nothing in `<head>` set it, and
`:root` carries the light tokens, so a dark reader, stored or by the OS, saw the light
chooser first and then a fade. The generated pages already set it before first paint. The
app's `<head>` now carries the same script, `THEME_HEAD_JS` from `build_pages.py`, held
byte-equal by `test_app_theme_head.py`, and the body's `applyTheme` call ignores a stored
value other than "dark" or "light" instead of writing it into `data-theme`. The themes,
their tokens and the toggle are unchanged. `ThemeBeforeTheBody` serves the page with every
body script removed and finds the stored dark theme on screen, and the OS deciding over an
invalid stored value, in Chromium and WebKit at 1200 px; 2 mutations, red in both.
`test_check_inline_js.py` now counts seven scripts.

### A stored preference of the wrong shape no longer stops boot (2026-09-25, v252)

Audit finding K10, the class of prior review #20 that the 2026-09-24 prefs fix left in
boot's prefetch. `{"fav":"city:Helsinki","cityIds":{"city:Helsinki":{}}}` made the loop
iterate an object and throw before anything was drawn: the chooser, "Näytöstietoja ei
juuri nyt saatu ladattua.", no language buttons, on every load. A `fav` that is not a
string threw the same way, there and in `knownArea`. The prefetch now takes a stored entry
only when it is a list and each id only when it is a string, and `knownArea` answers false
for anything but a non-empty string. A wrong shape costs the prefetch; the load validates
against the live groups as before. `MalformedStoredPrefs` (1200 and 375 px), Chromium and
WebKit; 3 mutations, red in both.

### A refused cache write no longer fails a data answer (2026-09-25, v253)

Audit finding K11. The data branch of `sw.js` awaited `cache.put` inside the promise that
answers a first fetch, so a write the storage refused (quota full) turned a 200 into a
network error for the page, and behind a cached copy it turned the check into a failure.
The write is now caught: the page gets the network's answer, nothing is stored, and
`fresh`, which tells the page to re-read the cache, is sent only when the write landed, so
a refused write cannot start a re-read of the old copy. `sw_fetch_harness.js` gained a
`put` that rejects; two cases in `test_sw_cache.py`; 2 mutations, both red.
`test_update_check.py`, which runs the real worker, passes in Chromium and WebKit.

### A failed genre or title read is tried again (2026-09-25, v254)

Audit finding K12. `ensureGenres` stored `{}` when `tmdb-genres.json` failed and
`ensureFilms` stored `{}` when `films.json` failed, so for the life of the tab every card
kept the provider's own genre strings and an English reader saw every Finnkino film under
its Finnish title; `films.json` was also never read again in a resumed tab. A failed read
now leaves nothing memoised (the film map keeps what it held), the sheet reads a map not
yet loaded as empty, and the resume refresh asks for the genre names and, in English,
re-reads `films.json`. `films-extra.json` already worked this way. No visual change.
`MetadataAfterAFailedRead` (1200 and 375 px, Chromium and WebKit): genre names and English
titles arriving on a resume, a later sheet reading the titles again, and a sheet opening
in Finnish while the file still fails; 5 mutations, red in both engines.

### The pages' merge key breaks words as the app's does (2026-09-25)

Audit finding E9. `merge_key` in `build_pages.py` carries the client's four `mergeKey`
patterns verbatim, and `test_city_merge.py` compared the pattern strings, but the same
pattern is not the same function: JavaScript's `\b` is ASCII and Python's is Unicode, so
"äsuomeksi" keyed as "ä" in the app and "äsuomeksi" on the pages. The two `\b` patterns now
compile with `re.A`; the other two keep Unicode `\s`, which is what JavaScript's `\s` is,
and `norm()` absorbs the whitespace the ASCII pair leaves. The client is unchanged, since
a lookbehind would be a syntax error on older Safari. No committed title keyed differently
and the pages regenerate unchanged. The new test runs the client's own `normTitle` and
`mergeKey` in node over seventeen hard cases and every committed title; 3 mutations, red.

### The venue lists arrive in two combined files, one per half (2026-09-26, v255)

Audit finding K6. Every load asked for one `data/venues-{id}.json` per provider, 82
requests plus 82 worker revalidations for 25 KiB, while the plain chooser uses none of it
until the picker opens ([research](../research/cold-load.md)). The single cloud-written
file first proposed was changed by the maintainer on 2026-09-26 to one file per half,
because a cloud-written file would have held the local half's venues and `pending` back
until the next cloud run. `data/venuelists-local.json` and `data/venuelists-cloud.json`
carry every provider file of their half verbatim (`scripts/providers/venuelists.py`);
`run.py` and `run_cloud.py` each rewrite their own half's file after a run, aborted runs
included, so each file has one writer. The client reads both and fetches a provider's own
file only when neither carries it; a missing or malformed combined file costs its own
providers a request each. The per-provider files stay written, the app's health and
freshness read the same fields as before, and the status page is unchanged. Drift:
`scripts/build_venuelists.py` in the Checks regeneration step and
`tests/test_venuelists.py` against the committed provider files. `robots.txt` allows the
new family. Measured, Chromium cold/warm: chooser 89/87 -> 9/7, venue 90/88 -> 10/8,
Helsinki 115/103 -> 35/23; WebKit 90 -> 10, 91 -> 11, 109 -> 29. Tests:
`test_venuelists.py`, `test_venuelists_client.py` with `venuelists_harness.js`, two in
`test_cloud_pool.py`, `CombinedVenueLists` in the browser suite at 1200 and 375 px in
Chromium and WebKit; 12 mutations red, and a thirteenth equivalent, whose guard was
removed.

### The status page reads the two combined venue files (2026-09-26)

On the maintainer's instruction, as a separate change from the app's. `makeStatusStore`
read one `venues-{id}.json` per provider for its health table. Its load now reads
`providers.json`, the two combined files and `areas.json`; a provider neither combined file
carries, which is the whole half when one file is missing or malformed, is read from its
own file. The choice is the app's `venueFilesFrom`, copied into the store and held equal to
the app's by `test_status_store.py`. Finnkino still comes from `areas.json`. Live refresh is
unchanged in shape: a worker message for a combined file reads it from Cache Storage and
puts each of its providers through the same `notBehind` and `key` tests a single file gets,
never the network; the message filter admits the new family. Each provider's row reads the
same as from its own file. Requests, Chromium cold/warm 86/86 -> 6/6, WebKit 86/87 -> 6/7.
Tests: five scenarios in `status_store_harness.js` and the parity check,
`StatusPageCombinedVenueLists` in the browser suite at 1200 and 375 px in Chromium and
WebKit; three request counts in `test_status_store.py` now count the two combined-file
requests a load makes in a world without them. 6 mutations, all red.

### The status page reads a stored `null` preference as empty (2026-09-26)

Prior review #20, left on the status page when the app was fixed on 2026-09-24. Its
`prefs.get` returned what `JSON.parse` gave, so a stored `null` made `prefs.get().lang`
throw at boot and the page stayed on "Tarkistetaan…" with no language buttons. It now
carries the app's guard between the same marker comments, and `tests/prefs_harness.js`
takes the page as an argument, so `test_prefs_store.py` runs the same cases against both.
`StatusPageStoredNull` in the browser suite, 1200 and 375 px, Chromium and WebKit. 3
mutations red in the harness, the old block red in both engines.

### The status and privacy pages link an icon that exists (2026-09-26)

Both linked `/favicon.svg`, which this repository never held (`git log --all` finds no
such file), so every load of either answered a 404. They now name `/icon-192.png`, the
icon the app and the generated pages use; no new artwork. `tests/test_page_assets.py`
checks every icon, manifest and preload link on every tracked page against the checkout;
3 mutations red.

### A list a chain filter emptied says so (2026-09-26, v256)

Prior review #27. `emptyMsg` checked the chips and the search but not `state.chains`, so
with Finnkino isolated in a Helsinki city view, 16.9., which only Orion plays, read
"Valitussa teatterissa ei ole näytöksiä ke 16.9." above "Suodattimet: Finnkino".
Reproduced in Chromium and WebKit before the change. A chain filter now counts as
`anyFilter()` already counted it, and the line is the filter message. Leffat's empty list
also draws the chain legend Ajat's empty list already drew, the existing element
unchanged. On a day only one chain plays the legend still has one entry and is not drawn;
"Tyhjennä suodattimet" is the undo there. `ChainFilterEmptiesTheList`, 1200 and 375 px,
Chromium and WebKit; 2 mutations, red in both.

### The automatic jump past an empty today is not the reader's choice (2026-09-26, v257)

Prior review #28. With nothing on today at all, `loadSchedule` moved to the next day with
shows through `selectDay`, which tracked `date_changed` and saved the day in `kino-prefs`
although the reader chose nothing. Reproduced in Chromium and WebKit: Orion without its
14.9. rows opened on 16.9. and stored `{"day":"2026-09-16"}`. `selectDay` now tracks and
saves and then calls `showDay`, which moves the list and the chips; the jump calls
`showDay` alone. A chip, calendar or next-day pick behaves as before. Chips are built
before the load, so none reads the unsaved day. `test_widen_load.py:56` asserted the saved
day on the jump; that line now asserts none. `widen_load_harness.js` records tracked
events; `AutomaticDayJump`, 1200 and 375 px, Chromium and WebKit. 4 harness mutations red,
3 browser mutations red in both engines.

### Leffat sorts by the title the card shows (2026-09-26, v258)

Prior review #29. The list sorted on `title`, the Finnish title, while English mode draws
`disp()`, the English or original title, so an English list was out of order. It now
sorts on `disp()`; Finnish and Swedish, where `disp()` is `title`, keep their order. The
Ajat view sorts by time and is unchanged. `FilmOrderFollowsTheShownTitle`, 1200 and 375
px, Chromium and WebKit, with the Finnish order as the counterweight; the old key red in
both engines on the English case only.

### A chain picked on another day adds to the pick (2026-09-26, v259)

The rest of prior review #27. `toggleChain` cleared the filter when the pick's size equalled
the day's chain count, so a chain picked on another day inflated it. Reproduced in
Chromium and WebKit with three Helsinki chains: Finnkino isolated on 14.9., then one click
on Orion on 16.9. (Orion and Riviera) showed both, dropped the Finnkino pick and hid
"Kaikki". The count is now over the day's chains: the filter clears when a click leaves
none of them picked, or when the pick is exactly the day's chains, and a pick from another
day stays. `ChainPickOnAnotherDay`, Leffat and Ajat, 1200 and 375 px, Chromium and WebKit;
3 mutations, red in both.
